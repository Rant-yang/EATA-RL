...
'''
Enveloped RSI
探索交易指标：用布林带创建全新的RSI指标——eRSI - 汇商传媒的文章 - 知乎
https://zhuanlan.zhihu.com/p/342624325
在RSI-High上应用指数平均线的bolling upper band： RSI - High中减去upper band 
RSI-Low上应用指数平均线的bolling lower band：RSI - Low中减去lower band
操作策略：
- 初始RSI的周期是3。增强的布林带则是5，并且标准差为1。
- 当RSI-Low线下沉到0以下且前面两个值大于0时，买入。
- 当RSI-High线下沉到0以下且前面两个值大于0时，卖出。
- 风险管理是基于平均真实范围(ATR)。
'''
